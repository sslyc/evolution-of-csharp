...
- 可以支持LINQ（C#3.0引入，且LINQ建立在迭代器之上）。LINQ可以算得上CSharper的快乐源泉。
...
> 作为C#开发者，首次接触Java时，非常容易犯的错误就是用`==`来判断字符串相等。因为Java中对于引用类型，`==`事实上仅仅判断了两个引用对象是否指向同一块内存区域。在Java中，只能用equals方法进行字符串相等判断。这也是许多CSharper刚刚转Java时，吐槽最多的地方。
...
> To CSharper：做为对比，Java的说明注释为 /**开头， */结尾的一段。每一行都是\*开头，具体section则以@开头的。
...
同时，在C#3.0版本，由于有了通用委托，C#支持对匿名方法的自动类型推断。上述代码可简写作：

```csharp
var add = delegate (int a, int b) { return a + b; };  //var是C#3.0的隐式类型语法，由赋值语句的右侧表达式自动推断出变量类型。此句将自动推断出add类型为Func<int, int, int>
```
...
此时如果执行`TestForIterator()`方法，会按什么顺序打印呢？
...
```csharp
//接上例
IEnumerable<int> infinite = GetInfinite(); //infinite是迭代器
foreach (int integer in infinite)
{
...
这里事实上由编译器生成了属性背后的支撑字段，只是该字段并不能被使用者直接调用，而只能通过get和set方法操作该字段。通过反射可以看到，其背后的实现字段命名为`<类型>k__BackingField`。显然通过C#语法无法声明这种命名的字段，这样做可以避免跟用户声明的字段冲突。
...
> > 这再次应证了：Java可能语法不是很炫酷，但是我的库很强。语法不足用库来补。
...
Console.WriteLine(anonymousObject.Name);  //打印anonymousObject包含的Name属性
Console.WriteLine(anonymousObject.n);  //打印anonymousObject包含的n属性
```
...
> ```
>
> 上述代码在调用`test()`时，将把Lambda描述的内容作为`MyFunctionalInterface`接口中`method`方法的实现，传入`forLambda`方法。因此`forLambda`调用`impl`的`method`方法时，实际上调用了Lambda所描述的方法，也就是打印"Lambda测试"
>
> 此外，Java系统提供了一堆预定义的函数式接口，它们被定义在`java.util.function`包下。这些接口基本满足了Stream编程的需求（Java的Stream编程语法类似LINQ的查询扩展方法，本文会在介绍到LINQ的时候做出介绍）。
>
...
这里有必要特别絮叨一下`Expression\<TDelegate>`。它派生自LambdaExpression，用于描述一个Lambda的表达式。其中TDelegate虽然未指定泛型约束条件(主要原因是C#7.1之前并不支持Delegate作为泛型约束类型），但是一般为强类型委托类型（Func或者Action）。
...
方法语法使用查询扩展方法。它们是针对IEnumerable\<T>和IQueryable\<T>接口的一系列扩展方法。它们被定义在System.Linq命名空间下。

大部分扩展方法仍旧返回IEnumerable\<T>和IQueryable\<T>接口，因此大部分方法支持连缀，是“流式”的。
...
//泛型版本
public interface IQueryable<out T> : IEnumerable<T>, IEnumerable, IQueryable {} //out是协变关键词，C#4.0会讲
```

显然，它可以用作foreach迭代。

其中，ElementType为表达式树计算结果的返回类型，Expression存储的就是当前表达式树，Provider则是用于解析表达式树以提供真实数据的提供者。

先抛开这些具体字段的用法，我们已经隐隐感觉到，它跟表达式树密切相关了。

现在我们直接来看LINQ：

LINQ中针对IQueryable\<T>的查询扩展方法，几乎与IEnumerable\<T>的一一对应。唯一不同点是，IEnumerable\<T>的扩展方法中传递委托的地方，IQueryable\<T>传递的是同类型的委托作为泛型参数的表达式树类型Expression\<TDelegate>。
...
THINK BIG！！我可以不按照Lambda的描述来执行，而是按照我们自己的套路修改执行逻辑！
...
显然，表达式树不会用来干这个，我们不会这么无聊。

针对IQuerable\<T>的LINQ扩展方法可能返回的仍是IQuerable\<T>，它们可以连缀。在真正执行之前，连缀过程一直都是在构建表达式树而已。接口定义中的Expression便是存储了当前已构建的表达式树，Type则是当前表达式树执行后应当返回的类型，Provider便是负责根据表达式树的描述真正执行数据查询操作。
...
LINQ to Entity把LINQ的大部分扩展方法重写了一遍，以用于查询数据库。在Where等方法调用时，仅仅是构建表达式树，当迭代过程触发时，才会交由Provider拼接数据库查询语句，进行真的查库操作。因而我们可以这样执行数据库的查询：
...
> Java由于没有表达式树，因而它虽然支持了Stream编程，但无法支持表达式转换。
...
| 类型 | 访问修饰符 | 类型 | 是否可以自定义成员名称 | 是否支持解构(Deconstruct) | 是否支持表达式树 |
| --- | --- | --- | --- | --- | --- |
...
所谓表达式主体，就是用表达式表述方法的主体。该方法只有表达式所描述的一句话，表达式的值可作为方法的返回值。它简化了方法的定义，是C#又一次向函数式编程的看齐。
...
其次，C# 7.0以后，允许你等待任何类的对象，只需要它实现`GetAwaiter`方法。

`GetAwaiter`方法返回一个`INotifyCompletion`或者`ICriticalNotifyCompletion`接口的对象，该接口用于实现通知回调。

此时你就成功的定义了一个**自定义可等待类**，该类也是可等待的。返回该类型的方法也是可等待方法，也可以使用`async`/`await`关键词升级为异步方法。

### 异步方法构建器

现在，你还可以定义一个类。这个类需要满足具有以下条件：

- 它提供一个`static`的`Create`方法，返回一个该类的对象。
- 它提供一个`Start`方法，传递`IAsyncStateMachine`接口类型的状态机作为参数。并提供一个`SetStateMachine`方法用于设置状态机。
- 它提供`SetException`、`SetResult`方法用于设置异常、写入异步结果。
- 它提供`AwaitOnCompleted`、`AwaitUnsafeOnCompleted`方法用于描述执行到await语句之后应该怎么做。并确保在任务结束时，调用了`Awaiter`的对应结束方法，以及`StateMachine`的`MoveNext`方法。

此时，你就成功定义了一个**自定义异步方法构建器**。

你可以给你刚刚自定义的可等待类上添加特性`AsyncMethodBuilderAtrribute`，并在参数里指定使用`typeof(你定义的异步方法构建器)`。这样返回你定义的可等待类型的异步方法，会使用你定义的构建类替代系统默认的构建类，去完成异步方法的构建。
...
```csharp
public bool IsHttps(string url) => url switch    //在此例子中，方法使用了表达式主体，而主体正是swtich表达式
{
...
使用using进行自动释放的语法，现在多了一种。using可以直接作为一条语句，不用开启一个代码块了。使用using声明时，必须同步声明变量（这样变量拥有更明确的生命周期），同时需要去除using后面的圆括号，不再需要花括号，但是需要添加语句结束符号`;`。如果使用using声明，则会在当前语句所在代码块结束时自动释放，声明的变量的有效期也会持续到当前代码块结束。
...
开发者需要尽可能规范的实现，以便使用者不会产生误解。而作为使用者，需要看清具体定义，以免被坑。
...
这时，编译器会自动提供索引的支持，使用索引将实际调用`this[int]`。
...
这时，编译器会自动提供范围的支持，使用范围将实际调用`Slice(int, int)`。
...
作为CSharper或者Javaer，会发现C#和Java本同源，却走上了不同的发展道路。网上关于Javaer和CSharper的口舌战争实数不少，但是如果两群人可以相互张望一下，又会有什么趣事发生呢？

## 有什么是Java有但是C#没有的

这章是写给CShper的。（此章节中的对比部分，不会采用之前的格式。）

没有接触过Java的CSharper虽然耳闻C#是Java的改良品种，却不知道到底改良了些什么。通过上面的文章和其中的对比，相信已经能有所概念了。但是，Java难道就没有什么特性在C#中找不到对应的吗？

相信CSharper最想了解的就是这个了。
...
我看到过很多第三方组件，因为需要提供各个语言版本的SDK，所以自然少不了有C#版本。可能因为人员有限，无法由原生的CSharper完成编写。这些C#的版本SDK，一眼便能看出是Javaer写出来的。作为CShper使用起来会觉得有些许别扭，因为它们“满满的Java气息，不够C# Style”。之所以有这样的差别，源于Javaer和CSharper的思维习惯不同。我把CSharper具有的思维风格称作C# Style。该风格体现在代码里就是：简约、自然、直观、规范（规范这点与Java是一致的）。
...
后来该库被移植到了\.Net上，被命名为NPOI。然而，作为CSharper，其实非常用不惯这个库。为什么呢？我们来对比一下C#的库。
...
So，how to get一个CSharper的代码Style呢？如果你已经很熟悉Java了，现在因为看了我的文章跃跃欲试想上手写写C#的代码，或者因为生活所迫不得不写点C#的库，那么不妨按照下面的步骤试试，这能快速让你书写的C#代码看起来像原生的CSharper写的。
